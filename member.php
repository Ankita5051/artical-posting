...
///other
$sql="SELECT distinct(id),username,img_url,about from members where id not in(SELECT distinct(members.id) from freind,members where members.id='$id' or (members.id=user_id and frend_id='$id') or (user_id='$id' and members.id=frend_id)) and id!='$id'";
$res=mysqli_query($con,$sql);
$status="other";
while($row=mysqli_fetch_array($res)){
    ?>
